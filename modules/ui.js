...
const state = { currentMonth: new Date().toISOString().slice(0, 7), transactions: [], categories: [] };
...
function shiftMonth(delta) {
  ...
  updateMonthLabel(); render(); renderCharts();
}
...
function renderCategorySelect() {
  ...
  state.categories.filter(c => c.type === type).forEach(c => { sel.innerHTML += `<option value="${c.name}">${c.name}</option>`; });
}

function renderCategoryList() {
  const list = document.getElementById("categoriesList");
  if (!list) return;
  list.innerHTML = state.categories.map(c => `<div class="cat-item"><span>${c.name} <small>(${c.type})</small></span><button class="btn-del" onclick="window.deleteCat('${c.id}')">🗑</button></div>`).join('');
}

function renderTable(list) {
  const tb = document.querySelector("#transTable tbody");
  if (!tb) return;
  tb.innerHTML = list.length === 0 ? '<tr><td colspan="6" style="text-align:center;padding:20px;color:#999">Nessun movimento</td></tr>' : list.map(t => `
    <tr style="color:${t.tipo==='entrata'?'var(--success)':'var(--danger)'}">
      ...
      <td><button class="btn-sm" onclick="window.toggle('${t.id}')">${t.confermato?'Annulla':'Conferma'}</button><button class="btn-sm" style="color:red" onclick="window.del('${t.id}')">🗑</button></td>
    </tr>`).join('');
}
...
function renderMobileList(list) {
  ...
  if (!c) return;
  c.innerHTML = list.length === 0 ? '<p style="text-align:center;color:#999;padding:20px">Nessun movimento</p>' : list.map(t => `
    <div class="m-card ${t.tipo}">
      <div class="m-info"><h4>${t.cat} <small>(${t.membro})</small></h4><div class="m-actions"><button class="m-btn conf" onclick="window.toggle('${t.id}')">${t.confermato?'Annulla':'Conferma'}</button><button class="m-btn del" onclick="window.del('${t.id}')">Elimina</button></div></div>
...
function renderRollover(cong, cassa, utile) {
  document.getElementById("rolloverList").innerHTML = `<li>💰 Cassa: ${fmt(cassa)}</li><li>⚖️ Conguaglio: ${fmt(cong)}</li><li>📉 Utile: ${fmt(utile)}</li>`;
}
...
function setupEvents() {
  ...
  document.getElementById("btnAggiungi").addEventListener("click", async () => {
    ...
    try {
      ...
      if (saved) { state.transactions.unshift(saved); render(); renderCharts(); document.getElementById("fImporto").value = ""; }
...
window.deleteCat = async (id) => {
  if (confirm("Eliminare?")) { await deleteCategoryFromDB(id); state.categories = await loadCategoriesFromDB(); renderCategoryList(); renderCategorySelect(); }
};
window.addNewCategory = async () => {
  ...
  state.categories = await loadCategoriesFromDB();
  renderCategoryList(); renderCategorySelect();
...
window.toggle = async (id) => {
  ...
  if (t) { t.confermato = !t.confermato; t.reale = t.confermato; await updateTransaction(id, { confermato: t.confermato, reale: t.reale }); render(); }
};
window.del = async (id) => {
  if (confirm("Eliminare?")) { state.transactions = state.transactions.filter(x => x.id !== id); await deleteTransaction(id); render(); }
};
